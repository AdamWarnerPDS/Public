# From https://github.com/russelltomkins/Active-Directory/blob/master/Query-InsecureLDAPBinds.ps1
<#-----------------------------------------------------------------------------
Russell Tomkins
...
$OutPath = ".\InsecureLDAPBinds_" + "$HName" + "_$DateTime" + ".csv"


# Dump it all out to a CSV.
Write-Host $InsecureLDAPBinds.Count "records saved to $OutPath for Domain Controller" $ComputerName
$InsecureLDAPBinds | Export-CSV -NoTypeInformation $OutPath
# -----------------------------------------------------------------------------
# End of Main Script
# -----------------------------------------------------------------------------