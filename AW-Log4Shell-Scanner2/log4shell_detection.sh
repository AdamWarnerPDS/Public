...
printf '%s\n' ""
printf '%s\n' "--------------------------------------------------------------------------------"
printf '%s\n' "Log4Shell (CVE-2021-44228, CVE-2021-45046) Vulnerability Detection Script v$version"
printf '%s\n' "--------------------------------------------------------------------------------"
...
if [ "$output_filepath" = "" ]; then
  output_filepath="log4shell_detection.output.$hostname.$scan_ts.json"
fi
...
if [ ! -e "$search_root" ]
then
  printf '%s\n' "Usage: log4shell_detection.sh [search_root [output_filepath]]"
