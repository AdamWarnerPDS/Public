...
#####################################
# Modified by Adam Warner of Paragon Development Systems, original by Arun Sabale
#####################################
...
$getuser1 = $getalluser1.GetDirectoryEntry()
	...
	[string]$PassworLastSet = $getuser1.pwdLastSet
	[string]$AccountNeverExpires = $getuser1.isAccountNeverExpires
	[String]$LastLogonDate = $getuser1.lastlogondate
...
$object | add-member -MemberType NoteProperty -Name "isPasswordNeverExpires" -Value $AccountNeverExpires
$object | add-member -MemberType NoteProperty -Name "LastLogonDate" -Value $LastLogonDate
