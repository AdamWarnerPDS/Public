...
# Imported on 20220315T1246-0500
$countryCodes = ConvertFrom-CSV @"
...
$policyExclusions = @(`
    ...
    "No International Login",`
    "DOC: Require MFA for Admin",`
...
function Get-TrustedLocations {
    ...
    Get-AzureADMSNamedLocationPolicy `
        | Select Id,DisplayName,CountriesAndRegions `
        | Where-Object {$_.DisplayName -notin $locationExclusions} `
...
function New-NamedLocation {
    ...
    While ( $true ){
        $searchTerm = Read-Host -Prompt "Please enter a search term for the target country (at least three letters)"
        While ( $true ) {
            $result = $countryCodes | Select-Object -Property name,alpha-2 | Where-Object { $_.name -like "*$searchTerm*"}
            $result | Format-Table
            $selection = Read-Host -Prompt `
            "Enter the country code of the desired country (alpha-2), leave blank to search again, or `"exit`" to exit"
            #Write-Host "Selection is $selection" #debug text
            #Write-Host "Potential matches are:" #debug text
            $($result | Select-Object -ExpandProperty alpha-2) #debug text

            if ( $selection -in $($result | Select-Object -ExpandProperty alpha-2 ) ){
                # Verify selection
                $selectedCountry = $countryCodes | Select-Object -Property name,alpha-2 | Where-Object { $_.alpha-2 -eq "$selection"}
                $confirm = Read-Host -Prompt "Country $($selectedCountry.name) with code $($selectedCountry.alpha-2) is selected.  Is this correct? (y/n)"
                # Break out of selection if y
                if ( $confirm -eq "y" ){
                    break
                }
            }
        ...
        # Exit function on exit selection
        if ( $selection -eq "exit" ) {
            return
        }
        # break out of search if y
        if ( $confirm -eq "y" ){
            break
        }
    }
    ...
    # Slightly nicer country code list: https://en.wikipedia.org/wiki/ISO_3166-2
    $locationParam = @{
    ...
    #New-AzureADMSNamedLocationPolicy @$locationParam
}
...
function Get-ConditionalAccessPolicy {
    ...
    Get-AzureADMSConditionalAccessPolicy `
        ...
        | Where-Object {$_.DisplayName -notin $policyExclusions}
}
...
function Get-UserInfo {
    ...
    $script:userName = Read-Host -Prompt "Please enter a username in format `"user@domain.tld`" "
    $script:userInfo = Get-AzureADUser -objectId "$script:userName" | Select DisplayName,ObjectId,UserPrincipalName
}
...
function New-VacationPolicy {
    [CmdletBinding()]
    param()

    # Checks
    if ( $null -eq $($script:userInfo) ){
        Get-TargetUsername ; Get-UserInfo -user "$userName"
    }
    if ( $null -eq $($script:targetLocation) ) {
...
function Get-MainMenu {
    [CmdletBinding()]
    param()
    # Maybe remake like this? https://adamtheautomator.com/powershell-menu/
    $menuText = @"
    ==========================================================================
    Selections
    ==========================================================================
    ...
    ==========================================================================
    1. Create new vacation Conditional Access policy (Wizard)
    2. Change targeted user
    3. Display current vacation Conditional Access policies
    4. Delete a vacation Conditional Access policy
    5. Display current named locations
    6. Create new Named Location
    7. Delete a Named Location
    0. Exit
"@

    Write-Host $menuText
    $script:selection = Read-Host -Prompt "Please select an option"
    switch ($script:selection) {
        1 { New-VacationPolicy }
        2 { Get-UserInfo }
        3 { Get-ConditionalAccessPolicy }
        4 { Remove-ConditionalAccessPolicy }
        5 { Get-TrustedLocations }
        6 { New-NamedLocation }
        7 { Remove-NamedLocation }
        0 { break }
        Default {}
    }
}
...
try {
    ...
    Write-Host "No AzureAD Powershell session detected.  Please login to AzureAD"
    Connect-AzureAD
}
